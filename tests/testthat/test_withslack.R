...
# TEST BELOW REMOVED BECAUSE, WHILST TOTAL COST IS GUARENTEED TO BE IDENTICAL, 
# THE ALLOCATION (AND EVEN HIRING PROFILE) COULD CONCEIVABLE DIFFER. I.E. IN
# CIRCUMSTANCES WHERE THERE IS MORE THAN ONE OPTIMIMUM SOLUTION, IT CANNOT BE
# GUARENTEED THAT THE 'WITH' AND 'WITHOUT' SLACK RUNS WILL FIND THE SAME OPTIMUM.
...
#   expect_equal(resource_outputs_without_slack$FTE, resource_outputs_with_slack$FTE, tolerance = .001)
#   
...
#   expect_equal(alloc_outputs_without_slack$`Avg Sitting Day Cost`, alloc_outputs_with_slack$`Avg Sitting Day Cost`, tolerance = .001)
#   expect_equal(alloc_outputs_without_slack$`Avg Sitting Days`, alloc_outputs_with_slack$`Avg Sitting Days`, tolerance = .001)
#   expect_equal(alloc_outputs_without_slack$`Min Sitting Days`, alloc_outputs_with_slack$`Min Sitting Days`, tolerance = .001)
#   expect_equal(alloc_outputs_without_slack$`Max Sitting Days`, alloc_outputs_with_slack$`Max Sitting Days`, tolerance = .001)
#   expect_equal(alloc_outputs_without_slack$`Avg Sitting Days`, alloc_outputs_with_slack$`Avg Sitting Days`, tolerance = .001)
#   expect_equal(alloc_outputs_without_slack$`Total Sitting Days`, alloc_outputs_with_slack$`Total Sitting Days`, tolerance = .001)
#   expect_equal(alloc_outputs_without_slack$`Total Fee Cost`, alloc_outputs_with_slack$`Total Fee Cost`, tolerance = .001)
#   
# })
